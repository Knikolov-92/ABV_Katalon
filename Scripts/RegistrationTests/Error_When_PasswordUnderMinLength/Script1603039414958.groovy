...
for (def row = 1; row <= findTestData('PasswordLen').getRowNumbers(); row++) {
    WebUI.clearText(findTestObject('RegistrationPage/inputPassword'))

    WebUI.setText(findTestObject('RegistrationPage/inputPassword'), findTestData('PasswordLen').getValue('password', row))
    ...
    WebUI.verifyElementText(findTestObject('RegistrationPage/errorMessage'), findTestData('PasswordLen').getValue('errorMessage', 
            row))
}
